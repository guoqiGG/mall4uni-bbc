...
<template>
  ...
  <view :class="['Mall4j contenta', popupShowHiden ? 'page-hidden' : '']">
    ...
    >
      ...
      <!-- 轮播图 & 商品视频end -->
      <!-- 倒计时栏、预售 -->
      <countdown-bar v-if="((activityInfo.activityStatus && prodType===1 || prodType===2) || preSellStatus) && prodInfo" :preSellTime="preSellTime" :prodType="prodType" :prodId="prodId" :activityInfo="activityInfo" />
      <!-- 倒计时栏 -->
      ...
      <view class="prod-info">
        ...
        <view v-if="prodType == 2 && defaultSku.seckillPrice" class="prod-price">
          ...
          <text v-if="defaultSku.price > defaultSku.seckillPrice" class="original-price">￥{{ parsePrice(defaultSku.price)[0] }}.{{ parsePrice(defaultSku.price)[1] }}</text>
        </view>
        <!-- 拼团商品价格 -->
        <view v-else-if="prodType == 1 && defaultActivitySku.actPrice" class="goods-price">
          ...
          <view v-if="defaultActivitySku.price > defaultActivitySku.actPrice" class="original">
            {{ i18n.originalPrice }}
            <text class="original-price">￥{{ parsePrice(defaultActivitySku.price)[0] }}.{{ parsePrice(defaultActivitySku.price)[1] }}</text>
          </view>
        </view>
        <!-- 普通商品价格 -->
      ...
      <view v-if="deliveryModeVO && mold !== 1 && prodType != 5" class="sendway">
        ...
        <view class="coupon-con">
          <view v-if="deliveryModeVO.hasShopDelivery" class="sendway-item">
            <image src="/static/images/icon/allow.png" />{{ i18n.expressDelivery }}
          </view>
          <view v-if="deliveryModeVO.hasCityDelivery" class="sendway-item">
      ...
      <!-- 满减折 || 赠品 -->
      <view v-if="(prodDiscountList.length && preSellStatus != 1 && (!activityInfo||Object.keys(activityInfo).length===0)) || giveawayList.length || (prodType == 2 && activityInfo.activityStatus == 1 && (prodDiscountList.length || giveawayList.length))" class="discount" @tap="clickDiscount">
        <view class="coupon-tit">{{ i18n.promotion }}</view>
        <!-- 满减折 -->
        <view v-if="prodDiscountList.length && preSellStatus != 1 && (!activityInfo||Object.keys(activityInfo).length===0) || (prodType == 2 && activityInfo.activityStatus == 1)" class="coupon-con discount-con">
      ...
      <!-- 领券 -->
      <view v-if="couponList.length && prodType != 5 && prodType != 2 && prodType != 3 || (prodType == 2 && activityInfo.activityStatus == 1 && couponList.length)" class="coupon" @tap="showPopup">
      ...
      <view v-if="joinGroupList.length" class="spell-infor">
        ...
        <view v-for="(item, groupTeamId) in joinGroupList" :key="groupTeamId" class="spell-infor-content">
          <!-- 头像 -->
          <view class="head-img">
            <image :src=" item.sharePic ? item.sharePic : '/static/images/icon/head01.png' " @error="imageError(item,'sharePic')" />
          </view>
          <!-- 信息 -->
          <view class="spell-msg">
            ...
            <view class="spell-text">{{ i18n.lack }}
              ...
              {{ i18n.lack1 }} {{ item.endOfGroupTime.hou }}{{ i18n.time }}{{ item.endOfGroupTime.min }}{{ i18n.minute }}{{ item.endOfGroupTime.sec }}{{ i18n.second }}
            </view>
          </view>
          <!-- 凑团按钮 -->
          <view class="join-group" :data-groupteamid="item.groupTeamId" @tap="toSpellGroupDetail">{{ i18n.toGatherGroup }}</view>
        </view>
        <!-- 详情 -->
        <view class="rules clearfix">
          <text class="rules-text01">{{ i18n.groupInvitation }}{{ activityInfo.groupNumber }}{{ i18n.groupInvitationTips }}</text>
        </view>
      </view>
            ...
            <view v-for="(item, prodCommId) in littleCommPage" :key="prodCommId" class="cmt-item">
              <view class="cmt-user">
                ...
                <view class="cmt-user-info">
                  <image class="user-img" :src=" item.pic ? item.pic : '/static/images/icon/head04.png' " @error="imageError(item,'pic')" />
                  <view class="nickname">
                    <view class="name-star">
                      <view class="name">
                        {{ item.isWriteOff ? i18n.userOff : (item.isAnonymous == 1 ? i18n.anonymousEvaluation : item.nickName) }}
                      </view>
                      <view>
              ...
              <scroll-view v-if="item.pics && item.pics.length" class="cmt-attr" scroll-x="true">
                <image v-for="(commPic, index) in item.pics" :key="index" :src="commPic" :data-pics="item.pics" :data-src="commPic" @tap="clickImg(commPic,item.pics)" />
              </scroll-view>
            </view>
          </view>
          ...
          <view class="sl">
            ...
            <view class="shop-con">
              ...
              <view class="shop-fol">
                ...
                <view class="fol-num">
                  {{ shopInfo.fansCount < 10000 ? shopInfo.fansCount + i18n.haveSpacePeople : shopInfo.fansCount / 10000 + i18n.tenThousandPeople }}{{ i18n.follow }}
                </view>
              </view>
            </view>
          </view>
      ...
      <view class="cart-footer">
        ...
        <block v-if="hadOnloaded && prodType===0 || (prodType===1 && (!activityInfo.activityStatus || activityInfo.activityStatus==1) || (prodType===2 && !prodInfo.seckillVO))">
        ...
        <!-- 秒杀  或正在进行的团购 -->
        <block v-if="hadOnloaded && (prodType == 2 && activityInfo.activityStatus) || (prodType == 1 && activityInfo.activityStatus==2 )">
          <view v-if="activityInfo.activityStatus === 1 || prodType === 1" class="btn alone-buy cart-radius" @tap="showSku(1)">
            <text>{{ activityInfo.activityStatus === 1 && prodType == 2 ? i18n.retailPricePurchase:i18n.individualShopping }}</text>
          </view>
          <view v-if="activityInfo.activityStatus === 2" :class="[ 'btn group-buy cart-radius', activityInfo.activityStatus === 1 ? 'gray-btn' : '', ]" @tap="showGroupSku">
                ...
                <view class="discount-item">
                  ...
                  <view class="giveaways-name" @tap="toDiscountList(item.discountId)">
                    <view class="discount-content">
                      <text v-if="item.discountType">{{ i18n.every }}</text>
                      <text>{{ item.discountName }}，{{ i18n.maximumDiscount }}{{ item.maxReduceAmount }}{{ i18n.yuan }}</text>
                    </view>
      ...
      <!-- #ifdef MP-WEIXIN -->
      <view
        v-if="liveRoomParams && liveRoomParams.length && liveRoomParams[0].liveStatus === 101"
        class="live"
    ...
    <view v-if="commentShow" :class="prodCommData.number ? 'cmt-popup' : 'cmt-no-popup'">
      ...
      <view class="cmt-cont">
        <view class="cmt-tag">
          <text :class="evaluate == -1 ? 'selected' : ''" @tap="getProdCommPage(-1)">{{ i18n.all + " " }}({{ prodCommData.number }})</text>
          <text :class="evaluate == 0 ? 'selected' : ''" @tap="getProdCommPage(0)">{{ i18n.praise + " " }}({{ prodCommData.praiseNumber }})</text>
          <text :class="evaluate == 1 ? 'selected' : ''" @tap="getProdCommPage(1)">{{ i18n.mediumEvaluation + " " }}({{ prodCommData.secondaryNumber }})</text>
          <text :class="evaluate == 2 ? 'selected' : ''" @tap="getProdCommPage(2)">{{ i18n.badEvaluation + " " }}({{ prodCommData.negativeNumber }})</text>
          <text :class="evaluate == 3 ? 'selected' : ''" @tap="getProdCommPage(3)">{{ i18n.havePictures + " " }}({{ prodCommData.picNumber }})</text>
        </view>
        <view class="cmt-items">
          <view v-for="(item, prodCommId) in prodCommPage.records" :key="prodCommId" class="cmt-item">
            <view class="cmt-user">
              ...
              <view class="cmt-user-info">
                ...
                <view class="nickname">
                  <view class="name-star">
                    <view class="name">{{ item.isWriteOff ? i18n.userOff : (item.isAnonymous == 1 ? i18n.anonymousEvaluation : item.nickName) }}</view>
                    <view>
                      <comm-star :value="item.score" />
                    </view>
                  </view>
            ...
            <scroll-view v-if="item.pics && item.pics.length" class="cmt-attr" scroll-x="true">
              <ImgShow v-for="(commPic, index) in item.pics" :key="index" :class="['cmt-attr-img']" :src="commPic" @handleTap="()=>clickImg(commPic,item.pics)" />
            </scroll-view>
        ...
        <EmptyAllTips
          v-if="isLoaded"
          :emptyImg="4"
          :isEmpty="!prodCommPage.records.length"
...
export default {
  ...
  onShareAppMessage: function(res) {
    ...
    return {
      title: this.prodName,
      path: '/pages/prod/prod?prodId=' + this.prodId + '&isShare=1' + (res.from === 'button' ? '&cardno=' + cardno : ''),
      imageUrl: this.pic,
      desc: this.brief
    }
  },
  ...
  methods: {
    ...
    toDiscountPackageDetail(id) {
      uni.navigateTo({
        url: '/pages/discount-package-detail/discount-package-detail?comboId=' + id + '&shopId=' + this.shopId
      })
    },
      ...
      const params = {
        ...
        callBack: (res) => {
          ...
          this.setData({
            ...
            pic: res.pic,
            shopId: res.shopId,
            prodType: prodType, // 商品类型(0普通商品 1拼团 2秒杀 3积分 5活动商品)
            mold: res.mold, // 1虚拟商品
    ...
    buyNow(orderType) {
      ...
      util.checkAuthInfo(() => {
        ...
        let url = `/pages/submit-order/submit-order?orderEntry=1&orderType=${orderType}&mold=${this.mold}`
        if (orderType === 3) {
          url += '&dvyType=1'
        }
        if (orderType == 1) {
          ...
          const params = {
            ...
            callBack: res => {
              url = `/pages/submit-order/submit-order?orderEntry=1&orderType=${orderType}&orderPath=${res}&mold=${this.mold}`
    ...
    scorePay() {
      ...
      const params = {
        ...
        callBack: (res) => {
          ...
          if (res.actualTotal) {
            const url = `/pages/submit-order/submit-order?orderEntry=1&orderType=3&mold=${this.mold}`
    ...
    getShareQRCode() {
      ...
      // #ifdef H5 || APP-PLUS
      const code = `${config.domainAddress}/pages/prod/prod?prodId=${this.prodId}&cardno=${cardNo}`
    ...
    downloadImg() {
      ...
      wx.downloadFile({
        ...
        url:
          config.domain +
          '/p/distribution/qrCode/invitation?page=pages/prod/prod&scene=' +
          this.prodId +
          ',' +
          cardno,
        success: (res) => {
