...
<template>
  <view class="Mall4j htz-image-upload-list">
    <view v-for="(item,index) in uploadLists" :key="index" class="htz-image-upload-Item">
      <view v-if="isVideo(item)" class="htz-image-upload-Item-video">
        <!-- #ifndef APP-PLUS -->
        <video :disabled="false" :controls="false" :src="getFileUrl(item)">
          <cover-view class="htz-image-upload-Item-video-fixed" @tap="previewVideo(getFileUrl(item))" />
        ...
        <!-- #endif -->
        <!-- #ifdef APP-PLUS -->
        <view class="htz-image-upload-Item-video-fixed" @tap="previewVideo(getFileUrl(item))" />
        <image class="htz-image-upload-Item-video-app-poster" mode="widthFix" :src="appVideoPoster.indexOf('http') === -1 ? picDomain+appVideoPoster : appVideoPoster" />
        <!-- #endif -->
      </view>

      <image v-else :src="getFileUrl(item)" @tap="imgPreview(index)" />

      <view v-if="remove" class="htz-image-upload-Item-del" @tap="imgDel(index)"> ×</view>
    </view>
    <view
    ...
    <view v-if="previewVideoSrc!=''" class="preview-full">
      <video :autoplay="true" :src="previewVideoSrc.indexOf('http') === -1 ? picDomain+previewVideoSrc : previewVideoSrc" :show-fullscreen-btn="false">
        <cover-view class="preview-full-close" @tap="previewVideoClose"> ×
        </cover-view>
      </video>
    </view>
...
export default {
  ...
  methods: {
    isVideo(item) {
      let isPass = false
      if ((!/.(gif|jpg|jpeg|png|gif|jpg|png|webp)$/i.test(item) && this.dataType == 0) || (this.dataType == 1 && item
        .type == 1)) {
        isPass = true
      }
      return isPass
    ...
    getFileUrl(item) {
      ...
      url = url.indexOf('http') === -1 ? picDomain + url : url
      // console.log('url', url)
      return url
    },
    previewVideo(src) {
    ...
    imgDel(index) {
      ...
      //       const delUrl = this.uploadLists[index]
      //       this.uploadLists.splice(index, 1)
      //       this.$emit('input', this.uploadLists)
      //       this.$emit('imgDelete', {
      //         del: delUrl,
    ...
    videoAdd() {
      ...
      const nowNum = Math.abs(this.uploadLists.length - this.max)
      // eslint-disable-next-line no-unused-vars
      const thisNum = (this.chooseNum > nowNum ? nowNum : this.chooseNum) // 可选数量
      uni.chooseVideo({
        ...
        success: (res) => {
          ...
          this.chooseSuccessMethod([res.tempFilePath], res)
    ...
    imgAdd() {
      ...
      uni.chooseImage({
        ...
        sourceType: this.sourceType,
        success: (res) => {
          this.chooseSuccessMethod(res.tempFilePaths, res)
          // console.log('tempFiles', res)
          // if (this.action == '') { //未配置上传路径
          // 	this.$emit("chooseSuccess", res.tempFilePaths);
          // } else {
          ...
          const uploadTask = uni.uploadFile({
            ...
            success: (uploadFileRes) => {
              ...
              if (typeof this.uploadSuccess === 'function') {
                const thisUploadSuccess = this.uploadSuccess(
                  uploadFileRes)
...
<style>
	...
	.htz-image-upload-Item-del {
		...
		border-radius: 0rpx 10rpx 0rpx 25rpx;
		line-height: 36rpx;
		text-align: center;
		top: 0;
		right: 0;
