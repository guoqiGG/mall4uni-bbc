...
export default {
  ...
  data() {
    return {
      ...
      orderType: 0, // 订单类型(0普通商品 1拼团 2秒杀 3积分)
      userAddr: null,
      orderItems: [],
      shopCartOrders: [],
      // 所有店铺的数据
        ...
        callBack: (res) => {
          ...
          if (this.orderType === 2) {
            ...
            if (res.duplicateError != null && res.duplicateError == 1) {
              ...
              this.$Router.replace({ path: '/pages/pay-way/pay-way', query })
            }
          }
        },
        errCallBack: (errMsg) => {
        ...
        const params = {
          ...
          callBack: res => {
            if (res) {
              ...
              this.$Router.replace({ path: '/pages/pay-way/pay-way?orderNumbers=' + seckillOrderNumber })
            } else {
              this.pollingTimes++
