...
export default {
  ...
  routeToAfterPay(isSuccess, dvyType, vipId, balanceId, orderNumbers, payType, orderType, ordermold) {
    ...
    if (isSuccess) {
      // 支付成功后的跳转
      url = balanceId ? '/package-user/pages/my-wallet/my-wallet' : dvyType == 2 ? '/pages/pay-success/pay-success?sts=1&orderNumbers=' + orderNumbers + '&dvyType=' + dvyType : '/pages/pay-result/pay-result?sts=1&orderNumbers=' + orderNumbers + '&payType=' + payType + '&orderType=' + orderType + '&ordermold=' + ordermold
    } else {
      // 支付失败的跳转
      url = balanceId ? '/package-user/pages/recharge-balance/recharge-balance' : '/pages/pay-result/pay-result?sts=0&orderNumbers=' + orderNumbers + '&payType=' + payType + '&ordermold=' + ordermold
    }
    uni.redirectTo({
      url: url
    })
